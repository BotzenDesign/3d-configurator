...
export function useQuote({
  ...
  const fetchQuote = useCallback(async () => {
    ...
    try {
      ...
      const supabaseUrl = WidgetConfig.supabaseUrl;
      const anonKey = WidgetConfig.supabaseAnonKey;

      if (!supabaseUrl || !anonKey) {
        throw new Error('Supabase environment variables missing in WidgetConfig');
      }
