...
export async function createCheckout(input: CreateCheckoutInput): Promise<CheckoutResponse> {
  ...
  const payload = {
    ...
    shop: WidgetConfig.shopDomain
  };

  // The request goes to the App Proxy configured in shopify.app.toml
  // Storefront URL: /apps/polar3d/checkout -> App Backend: /api/checkout
  const res = await fetch(`/apps/polar3d/checkout`, {
    method: 'POST',
    headers: {
