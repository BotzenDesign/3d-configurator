...
export default function PrintBed({ printType = "FDM", isOversized = false }: PrintBedProps) {
  ...
  const bedColor = isOversized ? "#ef4444" : "#555555";
  const gridColor = isOversized ? "#ef4444" : "#0b0909";
  ...
  return (
    <group ref={ref} position={[0, -0.5, 0]}>
      ...
      <group position={[0, 0.01, 0]}>
        ...
        {Array.from({ length: Math.floor(GRID_SIZE_Z / 10) + 1 }).map((_, i) => (
          <mesh key={`h-${i}`} position={[0, 0, (i * 10) - (GRID_SIZE_Z / 2)]}>
            ...
            <meshBasicMaterial color={gridColor} transparent opacity={isOversized ? 0.3 : 0.15} />
          </mesh>
        ))}
        {/* Vertical lines */}
      ...
      <Html position={[GRID_SIZE_X / 2, 2, GRID_SIZE_Z / 2]} center style={{ pointerEvents: "none" }}>
        <div style={{ background: isOversized ? "rgba(239, 68, 68, 0.85)" : "rgba(10, 10, 15, 0.85)", backdropFilter: "blur(8px)", border: `1px solid ${isOversized ? '#fca5a5' : '#ffffff40'}`, borderRadius: 6, padding: "2px 8px", fontSize: 11, fontWeight: 600, color: "#ffffff", fontFamily: "'Inter', -apple-system, sans-serif", whiteSpace: "nowrap", letterSpacing: "0.02em", boxShadow: `0 0 12px ${isOversized ? '#ef4444' : '#ffffff15'}` }}>
          {isOversized ? "OVERSIZED" : `${GRID_SIZE_X} x ${GRID_SIZE_Z} mm`}
        </div>
      </Html>
      ...
      <Html position={[-GRID_SIZE_X / 2, vol.h / 2, -GRID_SIZE_Z / 2]} center style={{ pointerEvents: "none" }}>
        <div className="flex flex-col items-center gap-1">
          <div className={`h-10 w-0.5 ${isOversized ? 'bg-red-500/50' : 'bg-[#22c55e]/50'}`} />
          <div style={{ background: "rgba(10, 10, 15, 0.85)", backdropFilter: "blur(8px)", border: `1px solid ${isOversized ? '#ef444440' : '#22c55e40'}`, borderRadius: 6, padding: "2px 8px", fontSize: 11, fontWeight: 600, color: isOversized ? "#ef4444" : "#22c55e", fontFamily: "'Inter', -apple-system, sans-serif", whiteSpace: "nowrap", letterSpacing: "0.02em", boxShadow: `0 0 12px ${isOversized ? '#ef444415' : '#22c55e15'}` }}>
            Max H: {vol.h}mm
          </div>
          <div className={`h-10 w-0.5 ${isOversized ? 'bg-red-500/50' : 'bg-[#22c55e]/50'}`} />
        </div>
